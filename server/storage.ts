...
export const storage = {
  // Quotes
  async insertQuote(quote: schema.QuoteFormValues) {
    try {
  ...
  // Contacts
  async insertContact(contact: schema.ContactFormValues) {
    try {
  ...
  // Subscriptions
  async insertSubscription(email: string) {
    try {
      // Check if email already exists
      const existingSubscription = await db.query.subscriptions.findFirst({
        where: (subscriptions, { eq }) => eq(subscriptions.email, email)
      });
  ...
  // Blog Posts - in a real app, these would be stored in the database
  // For now, we're returning mock data that would be in the database
  getBlogPosts() {
    return [
      {
        id: "1",
